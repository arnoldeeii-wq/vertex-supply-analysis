...
-- 2. top 3 products with the highest profits





-- 3. best product catergory
...
SELECT
    category,
    total_profit,
    RANK() OVER (ORDER BY total_profit DESC) AS category_rank
FROM top_3_products;

-- total profits made from kitchen items gross over $1,606,462. top performers , followed by office supplies at $1,488,650


-- 4. warehouse handling largest inventory


select warehouse, sum(quantity_received) inventory
from shipments
group by warehouse
order by sum(quantity_received) desc;

-- The 5 warehouses have an almost equal split of produscts held, with ibadan leading the pack @ 59,648 in store, and 59,458 in Abuja's stock

-- 5. how transportation affects profitability

select selling_price-unit_cost as profit, 
(selling_price-unit_cost) * quantity_received as true_profit, 
((selling_price-unit_cost) * quantity_received) - transport_cost as tranport_cost_implication,
	case 
		when ((selling_price-unit_cost) * quantity_received) - transport_cost < 0
...
-- the table shows the distribution of profis and loss as transport implies. 

-- consistent suppliers over time

SELECT
    supplier_name,
    supplier_rating,
   dense_rank() OVER (ORDER BY supplier_rating DESC) AS supplier_rank
FROM suppliers;


-- the last two questions are time-based, and the data required for analysis is absent from this doc.
